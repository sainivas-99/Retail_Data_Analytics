...
6. It seems that weekly sales is not highly dependent on any of these factors
...
```{r}
lin_reg <- update(lin_reg, .~ Size + Type +Dept + Store + MarkDown1 + MarkDown2 
                  + MarkDown3 + MarkDown4 + MarkDown5 + Holiday_Ref)
summary(lin_reg)
```

```{r}
predictions <- predict(lin_reg, newdata = test_data)
MSE <- sum((test_data$Weekly_Sales - predictions)^2)
cat("\n The sum of squared errors:",MSE)
```

```{r}
rsquared <- 1 - (sum((test_data$Weekly_Sales - predictions)^2) / sum((test_data$Weekly_Sales - mean(test_data$Weekly_Sales))^2))
...
features <- merged_data[, c("Size","Dept","Store","MarkDown1","MarkDown2","MarkDown3", "MarkDown4", "MarkDown5", "Holiday_Ref")]
target <- merged_data$Weekly_Sales

splitIndex <- sample(1:nrow(merged_data), size = 0.8*nrow(merged_data))
train_data <- merged_data[splitIndex, ]
test_data <- merged_data[-splitIndex, ]
```
...
```{r}
rf_model <- randomForest(x = train_data[, names(features)],
                         y = train_data$Weekly_Sales,ntree = 10, mtry = 6)
...
```
From the above, we see that Size, Department, Store, Markdowns and 
Holiday_reference are most important features in predicting weekly sales.This
inturn means:
...
2. Promotional activities, as indicated by Markdowns, play a crucial role in 
    increasing sales.
3. Holidays have a noteworthy impact, suggesting that adjusting strategies 
    around holiday seasons can be essential for maximizing sales.

#Model evaluation
```{r}
predictions <- predict(rf_model, newdata = test_data[, names(features)])
